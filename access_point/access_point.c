...
#define LED_TEST_BODY "<html><body style=\"background-color: #FFE4E1;\"><h1 style=\"color:red;\">Controle do LED 💡</h1>"\
"<p>O LED está <strong>%s</strong></p>"\
"<p><a href=\"?led=%d\"> %s</a>"\
"<hr>" \
...
float read_temperature() {
    adc_select_input(4);  // Sensor interno está no ADC 4
...
static int test_server_content(const char *request, const char *params, char *result, size_t max_result_len) {
    ...
    if (strncmp(request, LED_TEST, sizeof(LED_TEST) - 1) == 0) {
        ...
        printf("🌡️ a temperatura atual e %f\n\n",temp);
        fflush(stdout);
        // Generate result
        if (led_state) {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "ligado", 0, "desligar", temp);
            
        } else {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "desligado", 1, "ligar", temp);
...
int main() {
    stdio_init_all();
 // Inicializa ADC para leitura de temperatura e LED
